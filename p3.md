...
Além disso, `begin_atomic()` and `end_atomic()`, implementados respectivamente por `Thread::lock()` e `Thread::unlock()`, protegem a execução de `sleep()` e `wakep()`, e do acesso a _queue, implementados em `synchronizer.h`.
